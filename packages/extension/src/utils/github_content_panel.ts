// Webview panel to display the static markdown content produced by GitHub tools.
// Single column with final content only.
import * as vscode from 'vscode'
import { env } from './env'
import { getGithubContentSession } from './github_content_sessions'

export async function openGithubContentPanelByUid(uid: string): Promise<void> {
  let session = getGithubContentSession(uid)
  if (!session) {
  ...
  const panel = vscode.window.createWebviewPanel(
    'reliefpilot.githubContent',
    'Relief Pilot: GitHub Content',
...
<html lang="en">
  ...
  <link rel="stylesheet" href="${enhanceCssUri}" />
    <title>Relief Pilot: GitHub Content</title>
    <style>
  ...
  <body>
    <div class="layout">
      <div class="col">
        <p class="section-title">GitHub Tool Output (Markdown)</p>
