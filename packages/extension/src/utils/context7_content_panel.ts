// Webview panel to display the static markdown content produced by Context7 tools.
// Unlike ai_fetch_url progress panel this is a single column with final content only.
import * as vscode from 'vscode'
import { getContext7ContentSession } from './context7_content_sessions'
import { env } from './env'

export async function openContext7ContentPanelByUid(uid: string): Promise<void> {
  let session = getContext7ContentSession(uid)
  if (!session) {
  ...
  const panel = vscode.window.createWebviewPanel(
    'reliefpilot.context7Content',
    'Relief Pilot: Context7 Content',
...
<html lang="en">
  ...
  <link rel="stylesheet" href="${enhanceCssUri}" />
    <title>Relief Pilot: Context7 Content</title>
    <style>
  ...
  <body>
    <div class="layout">
      <div class="col">
        <p class="section-title">Context7 Tool Output (Markdown)</p>
